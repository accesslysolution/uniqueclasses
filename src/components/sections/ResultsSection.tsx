  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-slate-100">
        ...
        {/* Added a more colorful badge for the Exam type */}
        <div className="absolute top-3 left-3 bg-cta text-white px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-md">
          {student.exam}
        </div>

        {/* Added an achievement icon overlay */}
        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-md p-1.5 rounded-full text-primary shadow-sm">
          <Award size={14} />
        </div>
      </div>
      
      <div className="p-5">
        <h4 className="text-lg font-display font-bold text-slate-900 truncate group-hover/card:text-cta transition-colors">
          {student.name}
        </h4>
        <p className="text-xs text-slate-500 truncate mb-4">{student.school}</p>
        
        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
          {/* Highlighted the score with the Primary color */}
